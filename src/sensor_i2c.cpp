...
//Read multiple registers, return array (of length numBytes, NO ERROR CHECKING)
uint8_t* sensor_i2c::read_register_buffer(const uint8_t reg, int numBytes) {
	uint8_t readBuffer[numBytes];
	for (int i = 0; i < numBytes; i++) readBuffer[i] = 0;	//Overkill but just making sure
	i2c_write_blocking(bus, bus_addr, &reg, 1, true);
	i2c_read_blocking(bus, bus_addr, readBuffer, numBytes, false);
	return readBuffer;	//TODO: Confirm return-by-pointer array syntax shenaniganry
   						//I'm sure it'll get mad at [int numBytes], or something else
}
